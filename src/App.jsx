...
/* ─── PlantUML encoding (pako deflateRaw — même algo que le serveur officiel) ── */
function encodePlantUML(text) {
  const chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_";

  function enc3(b1, b2, b3) {
    const e = b => chars[b & 63];
    return e(b1>>2) + e(((b1&3)<<4)|(b2>>4)) + e(((b2&15)<<2)|(b3>>6)) + e(b3&63);
  }

  function encode64(data) {
    let r = "";
    for (let i = 0; i < data.length; i += 3)
      r += enc3(data[i], data[i+1]||0, data[i+2]||0);
    return r;
  }

  try {
    // pako.deflateRaw = zlib.compress()[2:-4] en Python = ce qu'attend PlantUML
    const compressed = window.pako.deflateRaw(text, { level: 9 });
...
async function callClaude(description, diagramType) {
  ...
  if (!apiKey) return EXAMPLES[diagramType];
  try {
...
rectangle "UMLify AI" {
  ...
  usecase "Generer diagramme" as UC3
  usecase "Telecharger PNG" as UC4
...
:Selectionner type de diagramme;
if (Quota atteint ?) then (oui)
  :Afficher page Upgrade;
  stop
endif
:Envoyer a Claude AI;
:Generer code PlantUML;
:Rendre image;
...
function AuthPage({ onLogin }) {
  ...
  const inp = {
    ...
    color:G.text, fontSize:14, fontFamily:"inherit",
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ width:"100%", maxWidth:400, padding:"0 20px" }}>
        ...
        <div style={{ background:G.bgCard, borderRadius:16, padding:28, border:`1px solid ${G.border}` }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {mode==="register" && (
              <div>
                ...
                <input style={inp} placeholder="Ton prénom" value={name} onChange={e=>setName(e.target.value)}/>
              </div>
            )}
            <div>
              <label style={{ fontSize:12, color:G.textSec, display:"block", marginBottom:5 }}>Email</label>
              <input style={inp} type="email" placeholder="toi@exemple.com" value={email} onChange={e=>setEmail(e.target.value)}/>
            </div>
            <div>
            ...
            <button onClick={submit} disabled={loading||!email||!password} style={{
              ...
              background:loading||!email||!password
                ? G.bgHover : `linear-gradient(135deg,${G.accent},${G.accentLight})`,
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <p style={{ color:G.textSec, fontSize:14, textAlign:"center", marginBottom:24 }}>
          Tu as atteint la limite de {FREE_LIMIT} diagrammes gratuits.
        </p>
...
export default function App() {
  ...
  const [selType, setSelType]       = useState("class");
  ...
  const dt = TYPES.find(t => t.id === selType);

  const handleGenerate = async () => {
    if (!inputText.trim()) return;
    if (user.plan === "free" && user.used >= FREE_LIMIT) { setShowPricing(true); return; }
    setLoading(true); setResult(null); setImgError(false);
    const code = await callClaude(inputText, selType);
    const imageUrl = plantUMLUrl(code);
    setResult({ code, imageUrl });
    setHistory(h => [{
      id:Date.now(), title:inputText.slice(0,40),
      type:selType, date:new Date().toLocaleDateString("fr-FR"), imageUrl, code,
    }, ...h]);
  ...
  const loadExample = (type) => {
    ...
    setResult({ code, imageUrl: plantUMLUrl(code) });
    setImgError(false);
  };
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:G.bg, fontFamily:"Inter, system-ui, sans-serif", color:G.text }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        textarea{resize:none;font-family:inherit} textarea::placeholder{color:#4A5070} textarea:focus{outline:none}
      ...
      <aside style={{
        ...
        {[{id:"generate",icon:"⚡",label:"Générer"},{id:"history",icon:"🕐",label:"Historique"}].map(item => (
          ...
          {user.plan==="free" ? (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:7, fontSize:12 }}>
                ...
                <span style={{ color:user.used>=FREE_LIMIT ? G.red : G.accent, fontWeight:600 }}>{user.used}/{FREE_LIMIT}</span>
              </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:9, padding:"6px 4px" }}>
          ...
          <button onClick={() => setUser(null)} style={{ background:"none", border:"none", cursor:"pointer", color:G.textMuted, fontSize:16, padding:4 }}>↩</button>
        </div>
      </aside>

      {/* MAIN */}
      <main style={{ marginLeft:sideW, flex:1, padding:"28px", overflowY:"auto" }}>
          ...
          <div style={{ animation:"fadeIn 0.3s ease", maxWidth:1100 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, alignItems:"start" }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  {[["text","📝 Description"],["code","💻 Code source"]].map(([id,lbl]) => (
                    <button key={id} onClick={() => setInputType(id)} style={{
                      ...
                      border:`1px solid ${inputType===id ? G.accent : G.border}`,
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  {TYPES.map(t => (
                    <button key={t.id} onClick={() => setSelType(t.id)} style={{
                      ...
                      border:`1px solid ${selType===t.id ? t.color : G.border}`,
                      background:selType===t.id ? t.color+"18" : G.bgCard,
                      color:selType===t.id ? t.color : G.textSec,
                ...
                <div style={{ background:G.bgCard, borderRadius:14, border:`1px solid ${G.border}`, overflow:"hidden" }}>
                  ...
                  <textarea
                    ...
                    placeholder={inputType==="text"
                      ? "Ex : Une bibliothèque avec Livres, Auteurs, Membres et Emprunts…"
                ...
                <div>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {[
                      ...
                      {l:"Bibliothèque", t:"class",    tx:"Gestion de bibliothèque avec Livre, Auteur, Membre, Emprunt"},
                    ].map(ex => (
                      <button key={ex.l} onClick={() => { setInputText(ex.tx); setSelType(ex.t); }} style={{
              ...
              <div style={{
                ...
                <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
                  {loading ? (
                    ...
                    <div style={{ animation:"fadeIn 0.3s ease", width:"100%" }}>
                      <div style={{ background:G.green+"15", border:`1px solid ${G.green}33`, borderRadius:12, padding:14, marginBottom:12 }}>
                        ...
                        <pre style={{
                          ...
                          overflow:"auto", maxHeight:260, lineHeight:1.6, whiteSpace:"pre-wrap", wordBreak:"break-word",
                        }}>{result.code}</pre>
                      </div>
                    </div>
                  ) : (
                ...
                {result && (
                  <div style={{ padding:"10px 16px", borderTop:`1px solid ${G.border}`, background:G.bg, display:"flex", gap:8, alignItems:"center" }}>
                ...
                {history.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{
                      ...
                      <span style={{ fontSize:11, fontWeight:600, color:t?.color, background:t?.color+"18", padding:"3px 9px", borderRadius:99 }}>{t?.label}</span>
